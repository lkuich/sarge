...
describe("Cloudflare WWW deployment config", () => {
  it("deploys and updates secrets with Astro's generated Wrangler config", () => {
    ...
    const workerWorkflow = readFileSync("../.github/workflows/cloudflare-worker.yml", "utf8");
    const wranglerConfig = readFileSync("wrangler.jsonc", "utf8");

    expect(rootPackageJson.scripts.build).toContain("pnpm --dir www build");
    ...
    expect(workflow).toContain("pnpm --dir www test");
    ...
    expect(workerWorkflow).toContain('const zoneName = "sargetrack.app";');
    expect(workerWorkflow).not.toContain('const zoneName = "lkuich.com";');
