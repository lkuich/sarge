...
describe("Cloudflare WWW deployment config", () => {
  it("deploys and updates secrets with Astro's generated Wrangler config", () => {
    ...
    expect(workflow).toContain("npx wrangler deploy --config dist/server/wrangler.json --keep-vars");
    expect(workflow).toContain("npx wrangler secret put DATABASE_URL --config dist/server/wrangler.json");
